...
def bunch_of_crops(X, y, input_time_length, verbose=False):
    ...
    assert (len(new_X) == len(new_y)) and (
...
class EnsembleLayer(nn.Module):
    ...
    def forward(self, out_15, out_25, out_35, out_45, out_65, out_85):
        ...
        assert out_15.shape == out_35.shape
        assert out_15.shape == out_45.shape
        assert out_15.shape == out_65.shape
        assert out_15.shape == out_85.shape
        assert out_15.size(1) == 4
...
@verbose_func_name
def make_shallownet_trialwise(kernel_size, padding=0, verbose=True):
    model = nn.Sequential(OrderedDict([
        ("transpose_time_channel", TransposeTimeChannel()),
        ("temp_conv", nn.Conv2d(1,  40, kernel_size=(kernel_size,1), stride=(1,1), padding=padding)),
...
class EnsembleTrialwiseModel(nn.Module):
    def __init__(self):
        ...
        self.model_35 = make_shallownet_trialwise(kernel_size=35, verbose=False)
    ...
    def forward(self, x):
        ...
        out_25 = self.model_25(x) # (batch, 4)
        out_35 = self.model_35(x) # (batch, 4)
        out_45 = self.model_45(x) # (batch, 4)
...
class EarlyStopNoIncrease:
    ...
    def __call__(self, epoch_df):
        ...
        """
        assert (
            self.column_name in epoch_df.columns
...
def early_stop_reach_below(epoch_df, column_name, base_value):
    ...
    margin_of_error : float, [0,1]
    """
    assert column_name in epoch_df.columns, f"{column_name} not in epoch_df..."
...
def maxnorm_ensemble(model):
    maxnorm(model.model_15)
    maxnorm(model.model_25)
    maxnorm(model.model_35)
    maxnorm(model.model_45)
    maxnorm(model.model_65)
    maxnorm(model.model_85)
...
def exp(args):
    ...
    if (which_learning == "trialwise") and use_tied_loss:
        raise AttributeError("trialwise learning is not compatible with tied_loss")
    ...
    data_tr = butter_bandpass_filter(
        data_tr, lowcut=lowcut, highcut=highcut, order=order, verbose=True
    )
    data_tr = exponential_moving_standardize_from_braindecode(
        data_tr, factor_new=factor_new, init_block_size=init_block_size, verbose=True
    )
    ...
    data_te = butter_bandpass_filter(
        data_te, lowcut=lowcut, highcut=highcut, order=order, verbose=True
    )
    data_te = exponential_moving_standardize_from_braindecode(
        data_te, factor_new=factor_new, init_block_size=init_block_size, verbose=True
    )
    ...
    X_tr, y_tr, X_val, y_val = split_train_val(X_tr, y_tr, val_ratio=0.2, verbose=True)
    ...
    if which_learning == "trialwise":
        print("\ntrain + val")
        trial_dataset_tr_val = TrialDataset(
            torch.cat([X_tr, X_val]), torch.cat([y_tr, y_val]), verbose=True
        )
    elif which_learning == "cropped":
        print("\ntrain")
        crop_dataset_tr = CropDataset(
            X_tr, y_tr, input_time_length=input_time_length, verbose=True
        )
        print("\ntrain + val")
        crop_dataset_tr_val = CropDataset(
            torch.cat([X_tr, X_val]),
            torch.cat([y_tr, y_val]),
            input_time_length=input_time_length,
            verbose=True,
        )
    print("\ntrain")
    trial_dataset_tr = TrialDataset(X_tr, y_tr, verbose=True)
    print("\nval")
    trial_dataset_val = TrialDataset(X_val, y_val, verbose=True)
    print("\ntest")
    trial_dataset_te = TrialDataset(X_te, y_te, verbose=True)
    ...
    if which_learning == "trialwise":
        # for first training
        first_training_dataloader = DataLoader(
            trial_dataset_tr, batch_size=batch_size, shuffle=True
        )
        # for second training
        second_training_dataloader = DataLoader(
            trial_dataset_tr_val, batch_size=batch_size, shuffle=True
        )

    elif which_learning == "cropped":
        # for first training
        first_training_dataloader = DataLoader(
            crop_dataset_tr, batch_size=batch_size, shuffle=True
        )
        # for second training
        second_training_dataloader = DataLoader(
            crop_dataset_tr_val, batch_size=batch_size, shuffle=True
        )

    # for evaluation
    trial_dataloaders = {
        "tr": DataLoader(trial_dataset_tr, batch_size=batch_size, shuffle=False),
        "val": DataLoader(trial_dataset_val, batch_size=batch_size, shuffle=False),
        "te": DataLoader(trial_dataset_te, batch_size=batch_size, shuffle=False),
    }
    ...
    for i in range(1, 1 + args.repeat):
        ...
        early_stop_no_increase = EarlyStopNoIncrease(
            column_name="val_acc", patient_epochs=patient_epochs
        )
        loss_function = LossCase(
            which_learning=which_learning, use_tied_loss=use_tied_loss
        )
        ...
        if args.use_early_stop is True:
            ...
            for epoch in range(0, max_epochs):
                ...
                evaluation_val = evaluate_with_dataloader(
                    model,
                    trial_dataloaders=trial_dataloaders,
                    mode="val",
                    use_tied_loss=use_tied_loss,
            ...
            for epoch in range(
                save_best_model.saved_epoch + 1, save_best_model.saved_epoch * 2
            ):
                # Train
                model.train()
                ...
                # Early stop
                if early_stop_reach_below(
                    epoch_df, column_name=stop_column_name, base_value=loss_to_reach
                ):
                    print("\nSecond Early Stop")
                    break
            # else:
        ...
        elif args.use_early_stop is False:
        ...
        result_name = f"{args.result_dir}/{save_name}_subject{subject}_try{i}"
